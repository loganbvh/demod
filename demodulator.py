...
def dataload_lanl_csv(osc_file):
	...
	osc_dict = {'file': osc_file, 'dt_fast': df_time[5][0], 'tdo': df_tdo['PDO'], 'dt_slow': df_time[9][0], 'Field': df_tdo['Field_fixed']}
	return osc_dict
...
def spline_plot(demod_dict, plot=True, updown=True, delta=True, amp=True):
	...
	tck = interpolate.splrep(demod_dict['time_Field'], demod_dict['Field']) # find spline representation
	field_cs = interpolate.splev(time_cs, tck, der=0) # evaluate spline representation
	if amp:
	...
	if amp:
		df_spline = pd.DataFrame({'time': time_cs, 'Field': field_cs, 'freq': freq_cs, 'amp': amp_cs})
	...
	if plot and updown:
		...
		if not delta:    
			plt.plot(df_spline.Field.iloc[:maxidx], df_spline.freq.iloc[:maxidx]*1e-6, label='Up sweep')
			plt.plot(df_spline.Field.iloc[maxidx:], df_spline.freq.iloc[maxidx:]*1e-6, label='Down sweep')
			plt.ylabel('Frequency (MHz)')
		else:
			plt.plot(df_spline.Field.iloc[:maxidx], df_spline.freq.iloc[:maxidx]*1e-3, label='Up sweep')
			plt.plot(df_spline.Field.iloc[maxidx:], df_spline.freq.iloc[maxidx:]*1e-3, label='Down sweep')
	...
	if plot and amp:
		if updown:
			maxidx = df_spline.Field.argmax()
			plt.plot(df_spline.Field.iloc[:maxidx], df_spline.amp.iloc[:maxidx]*1e3, label='Up sweep')
			plt.plot(df_spline.Field.iloc[maxidx:], df_spline.amp.iloc[maxidx:]*1e3, label='Down sweep')
